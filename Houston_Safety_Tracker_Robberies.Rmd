...
<bignumber>`r prettyNum(robberies_city$total22, big.mark=",")`<br></bignumber><p>
people have been robbed.</p>
</div>
<div class="card" style="background-color:#99a0a5;">
<p>Over the last 12 momths,<br>
<bignumber>`r prettyNum(robberies_city$last12mos, big.mark=",")`<br></bignumber>
<p>people have been robbed.</p>
</div>
<div class="card" style="background-color:#99a0a5;">
...
<div class="card" style="background-color:#99a0a5;">
<p>The robbery rate is <br>
<bignumber>`r round(robberies_city$rate_last12,0)`<br></bignumber><p>
per 100K people the last year.</p>
</div>
</div>
<br>
About three years ago, the robbery rate in Houston was <h8>`r round(robberies_city$rate19,0)`</h8> per 100,000 residents - or just slightly less than the rate of deaths in motor vehicle crashes in Texas (12.5).

During the pandemic years, the risk of homicide in Houston rose above the risk of being killed in a traffic crash. The average robbery rate for the three-year period from 2019 to 2022 was <h8>`r round(robberies_city$rate_prior3years,0)`</h8> and last year it was <h8>`r round(robberies_city$rate21,0)`</h8>.

The situation, of course, is not the same neighborhood to neighborhood.

About <h8>`r round(pop_robberiesdown/2304580*100,0)`%</h8> of city residents live in neighborhoods where the robbery rate over the last 12 months is down or about the same as the three-year average. 

In fact, <h8>`r round(pop_robberieszero/2304580*100,0)`%</h8> of people live in neighborhoods where there were no robberies at all over the last 12 months.

But, about <h8>`r round(pop_robberiesup/2304580*100,0)`%</h8> of Houston residents live in neighborhoods where the robbery rate is up more than 20% in the last 12 months when compared to the three-year average from 2019 to 2021.

And, <h8>`r round(pop_robberiesmonthly/2304580*100,0)`%</h8> of us now live in neighborhoods where there was at least one robbery every month over the last year.

Across the city the last 12 months, there were about <h8>`r round(robberies_city$last12mos/52,0)`</h8> robberies every week. In 2019, that number was <h8>`r round(robberies_city$total19/52,0)`</h8> per week

<subhead>Houston robberies neighborhood by neighborhood</subhead>

ABC13 looked at the Houston Police Department's data, beat by beat across the city and is providing this look at the robbery trends by neighborhood for each of the last four years in the map below. The color-coding of the map is based on the robbery rate over 2019, 2020 and 2021 combined, according to ABC13's analysis of data the city police department publishes monthly.
...
"</td>
				...
				multi rows -->
				<td rowspan='5'>",
ifelse(robberies_beat$inc_prior3yearavgtolast12>=0,"Last 12<br>months up","Last 12<br>months down"),
"<br><chartbignumber>",
...
L.control.zoom({ position: 'topright' }).addTo(this)
  ...
  addLegend(opacity = 0.6,
            ...
            labels=c("Lowest Rate", 
                     ...
                     "Highest Rate"),
            position = "bottomleft", 
            title = "Robberies<br>Per 100K people<br>") 

houston_robbery_map
```

However, in the vast majority of neighborhoods in the city, the rate is much lower. In 17 of 118 of the Houston Police Department's, the robbery rate was zero last year. Filler, filler, filler.
<br>
<br>
