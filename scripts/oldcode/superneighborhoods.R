...
neighborhoods <- st_read("data/source/geo/HOUSTON_LIMITS_BOUNDARIES_PACKAGE.geojson") %>%
  st_transform(3857) %>% select(1:15) %>% janitor::clean_names() 
#%>%
#  rename("name"="SNBNAME")
# st_crs(beats)
...
zips <- zips %>% 
  st_filter(beats, .predicate = st_intersects)
...
neighborhood_map <- leaflet(options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.5, zoomDelta=0.5)) %>%
...
L.control.zoom({ position: 'topright' }).addTo(this)
  ...
  addPolygons(data = neighborhoods,
              color = "red", 
              popup = paste0("Name: ",neighborhoods$snbname),
...
#                                          minWidth ="200"),
  ...
  addPolygons(data = beats,
              ...
              opacity = 1, 
              fillOpacity = 0.2) %>%
  addCircleMarkers(data = houston_crime_last12, clusterOptions = 2)
# fillColor = ~council_active),

neighborhood_map

map_houston_crimes <- leaflet(houston_crime_last12) %>%
 # setView(-78.65, 35.79, zoom = 10) %>% 
  addProviderTiles(provider = "CartoDB.Positron") %>%
  addCircleMarkers(clusterOptions = 4)
map_houston_crimes
