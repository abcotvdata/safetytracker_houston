...
# Get demographic data and geography for Census counties
...
ourtx_places2 <- bind_rows(ourtx_places %>% filter(place!="Houston"),beats %>% st_transform((4269)))
# Add upgraded LA County districts to the new region wide police districts map
ourtx_places2$county <- ifelse(is.na(ourtx_places2$county),"Harris",ourtx_places2$county)
...
poppal <- colorBin("viridis", ourtx_places$population, bins = popbins)
poplabel <- paste(sep = "<br>", ourtx_places$place,ourtx_places$county,prettyNum(ourtx_places$population, big.mark = ","))
# Create map
ourtx_police_map <- leaflet(ourtx_places) %>%
...
# Set bins for beats pop map
popbins <- c(0,1000,5000,25000,50000,75000,100000,125000,150000,175000,200000,Inf)
poppal <- colorBin("viridis", ourtx_places2$population, bins = popbins)
poplabel <- paste(sep = "<br>", ourtx_places2$place,ourtx_places2$county,prettyNum(ourtx_places2$population, big.mark = ","))
# Create map
